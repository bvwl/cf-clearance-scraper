...
app.post('/cf-clearance-scraper', async (req, res) => {
    ...
    // 通过 finally 风格的成对增减记录并发任务数。这里当前代码没有使用 finally，
    // 因为每个分支都把异常吞掉并转换成 result；后续如改成直接 throw，需要补 finally。
    global.browserLength++

    switch (data.mode) {
        case "source":
            // 返回目标页面最终 HTML，适合只需要页面源码的场景。
            result = await getSource(data).then(res => { return { source: res, code: 200 } }).catch(err => { return { code: 500, message: errorMessage(err) } })
            break;
        case "turnstile-min":
            // 最小资源模式：拦截目标文档请求，替换成一个只渲染 Turnstile 的本地页面。
            result = await solveTurnstileMin(data).then(res => { return { token: res, code: 200 } }).catch(err => { return { code: 500, message: errorMessage(err) } })
            break;
        case "turnstile-max":
            // 完整加载模式：真实访问页面，在页面脚本里等待 turnstile 返回 token。
            result = await solveTurnstileMax(data).then(res => { return { token: res, code: 200 } }).catch(err => { return { code: 500, message: errorMessage(err) } })
            break;
